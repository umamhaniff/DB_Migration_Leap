...
# We can query kelurahan on demand or load it. Since kelurahan has 83k rows, we can load it.
...
def clean_wil_name(s):
    ...
    s = re.sub(r'^(kab\.|kabupaten|kota|kec\.|kecamatan|kel\.|kelurahan|desa|adm\.|adm)\s+', '', s)
    s = re.sub(r'\s+(kab\.|kabupaten|kota|kec\.|kecamatan|kel\.|kelurahan|desa|adm\.|adm)$', '', s)
...
print(f"Total siswa: {len(df_siswa)}")
