...
| siswa | idcalon | siswa | ~~id_calon~~ | ⏸️ **SKIP** — FK ke tabel calon belum tersedia di db_new. Mapping dihapus dari code. Backlog post-deadline. |
...
* **Tabel `rapor_siswa_file` & `rapor_lacak`**:
  * Mengatasi kendila nilai `id_rapor_siswa` dan `id_rapor_siswa_file` yang NULL. Solusinya adalah dengan melakukan pencocokan data `idsiswa` & `idjadwal` dari tabel file ke mapping ID rapor yang digenerate sebelum ekspor data.
...
* **Solusi**:
  ...
  3. Hasil pemetaan diekspor secara terpisah ke `fase_4/mapping_siswa.pkl` (juga disalin ke `fase_5/mapping_siswa.pkl` secara otomatis agar sinkron untuk rekan tim) dan `extract/cek_csv/mapping_siswa.csv` untuk keperluan audit manual.
...
### 🟢 Update 24 Juni 2026 (Sesi Sore): Fix Data Lapangan Tahap 2

#### 1. Penghapusan Mapping `id_calon` — ⏸️ SKIP
* Kolom `idcalon → id_calon` dihapus dari `patch_fase_4()` karena tabel calon di db_new belum tersedia.
* Kolom ini nullable → tidak blocking insert. Backlog post-deadline.

#### 2. Perbaikan `nomor_induk` Invalid (`siswa`)
* Fungsi `fix_no_induk` diperbarui: nilai `#N/A`, `0000`, `NODATAYET`, string kosong → `'-'`.
* Kasus spesifik: `S0000549` dengan `0000` dan `S0000522` dengan `00NF3` → `'-'`.

#### 3. Pembersihan `NODATAYET` di Beberapa Kolom (`siswa`)
* Setelah rename kolom, seluruh nilai `'NODATAYET'` di kolom `domisili`, `asal_sekolah`, `tingkat_sekolah`, `tempat_lahir`, `nomor_induk` diganti `'-'`.

#### 4. Drop Baris Orphan `K00017` di `kursus_siswa` — ✅ SELESAI
